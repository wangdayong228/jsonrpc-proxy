...
class RpcCacheDB {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.inner.get(sql, [ethHash], (err, row) => {
                if (err) {
                    ...
                    console.log(`getCfxHashByEthHash ${ethHash} duration ${duration}ms`);
    ...
    async setCorrectBlockHashBaseBlockNumber(baseBlockNumber) {
        baseBlockNumber = Number(baseBlockNumber);
        if (!baseBlockNumber) {
            throw new Error('baseBlockNumber is required and must > 0');
        }
        ...
        if (inUseBaseBlockNumber && inUseBaseBlockNumber <= baseBlockNumber) {
            logger.info(`skip setCorrectBlockHashBaseBlockNumber ${baseBlockNumber} because inUseBaseBlockNumber ${inUseBaseBlockNumber} <= ${baseBlockNumber}`);
            return;
        }
