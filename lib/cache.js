...
async function correctBlockHash(block) {
    if (block) {
        ...
        let ethHash = await db.getEthHashByCfxHash(block.rawHash);
        if (!ethHash) {
            ethHash = calculateBlockHash(block);
            await db.saveBlockHashMap(block.rawHash, ethHash);
        }
        block.hash = ethHash;

        // set parent hash
        block.rawParentHash = block.parentHash;
        let ethParentHash = await db.getEthHashByCfxHash(block.parentHash);
        if (!ethParentHash) {
            ...
            await db.saveBlockHashMap(block.rawParentHash, ethParentHash);
        }
