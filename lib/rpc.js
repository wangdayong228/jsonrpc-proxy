...
const { db, correctBlockHash } = require('./cache')
...
async function getBlockByHash(blockHash, includeTransactions = false) {
    if (CORRECT_BLOCK_HASH) {
        let rawHash = await db.getCfxHashByEthHash(blockHash);
        console.log(`get rawHash of computed hash(${blockHash}): ${rawHash}`);
        if (!rawHash) {
            return null
        }

        const block = await provider.send('eth_getBlockByHash', [rawHash, includeTransactions]);
